...
class EntailmentAttack(object):
    def __init__(self, model, embeddings, pop_size=4, max_iters=10, n1=8, n2=4):
        self.model = model
    ...
    def attack(self, x_orig, target):
        ...
        tmp = [glove_utils.pick_most_similar_words(x2_adv[i], self.embeddings, "euclidean", 50, 0.5) if x2_adv[i] != 0 else ([], [])
        ...
        tmp = [glove_utils.pick_most_similar_words(x2_adv[i], self.embeddings, "euclidean", self.n1, 0.5) if x2_adv[i] != 0 else ([], [])
               for i in range(len(x2_adv))]
        neighbours_list = [x[0] for x in tmp]
        neighbours_dist = [x[1] for x in tmp]
...
class GeneticAtack(object):
    ...
    def attack(self, x_orig, target, max_change=0.4):
        ...
        tmp = [glove_utils.pick_most_similar_words(
            x_orig[i], self.embeddings, "euclidean", 50, 0.5) for i in range(x_len)]
        ...
        tmp = [glove_utils.pick_most_similar_words(
            x_orig[i], self.embeddings, "euclidean", self.top_n, 0.5) for i in range(x_len)]
        neigbhours_list = [x[0] for x in tmp]
        neighbours_dist = [x[1] for x in tmp]
...
class PerturbSentBaseline(object):
    def __init__(self, sess, model, batch_model, dataset, embeddings,
    ...
    def perturb(self, x_cur, pos, x_orig, target):
        ...
        replace_list, _ = glove_utils.pick_most_similar_words(
            src_word, self.embeddings, "euclidean", 60)
...
class GreedyAttack(object):
    def __init__(self, sess, model, dataset, embeddings, lm):
        self.embeddings = embeddings
        self.sess = sess
        self.dataset = dataset
        self.model = model
